...
*******************************************************************************/
dev_t dev1 = 0;
static struct class *dev_class1;
...
static int __init my_driver_init(void)
{
        ...
        if((alloc_chrdev_region(&dev1, 0, 1, "my_Dev1")) <0){
                printk(KERN_INFO "Cannot allocate major number\n");
                return -1;
        }
        printk(KERN_INFO "Major = %d Minor = %d \n",MAJOR(dev1), MINOR(dev1));

        /*Creating cdev structure*/
        cdev_init(&my_cdev1,&fops);
        my_cdev1.owner = THIS_MODULE;
        my_cdev1.ops = &fops;

        /*Adding character device to the system*/
        if((cdev_add(&my_cdev1,dev1,1)) < 0){
            printk(KERN_INFO "Cannot add the device to the system\n");
            unregister_chrdev_region(dev1,1);
        }

        /*Creating struct class*/
        if((dev_class1 = class_create(THIS_MODULE,"my_class1")) == NULL){
            printk(KERN_INFO "Cannot create the struct class\n");
            unregister_chrdev_region(dev1,1);
        }

        /*Creating device*/
        if((device_create(dev_class1,NULL,dev1,NULL,"my_device1")) == NULL){
...
{
        device_destroy(dev_class1,dev1);
        class_destroy(dev_class1);
        cdev_del(&my_cdev1);
        unregister_chrdev_region(dev1, 1);
